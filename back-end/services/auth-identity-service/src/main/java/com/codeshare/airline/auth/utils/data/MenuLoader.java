...
public class MenuLoader {
    ...
    public void load(List<String> tenantIds) throws Exception {

        if (menuRepository.count() > 0) {
            log.info("✔ MenuLoader: Menus already exist — skipping load.");
            return;
        }

        log.info("⏳ MenuLoader: Creating menus for {} tenants...", tenantIds.size());
        ...
        for (String tenantIdString : tenantIds) {
            ...
            List<Menu> tenantMenus = buildMenusForTenant(tenantId);

            menuRepository.saveAll(tenantMenus);

            log.info("✔ MenuLoader: Inserted {} menus for tenant {}", tenantMenus.size(), tenantId);
        }
    }

    /**
     * Build fresh menu structure for a given tenant.
     */
    private List<Menu> buildMenusForTenant(UUID tenantId) throws Exception {

        List<Menu> menus = objectMapper.readValue(MENU_JSON, new TypeReference<>() {});

        List<Menu> allMenus = new ArrayList<>();

        for (Menu menu : menus) {
            processMenu(menu, null, tenantId, allMenus);
        }
    ...
    /**
     * Recursive processing of menu tree.
     */
    private void processMenu(Menu menu, Menu parent, UUID tenantId, List<Menu> collector) {
        ...
        if (menu.getChildren() != null) {
            for (Menu child : menu.getChildren()) {
                processMenu(child, menu, tenantId, collector);
            }
        }
    }
}
