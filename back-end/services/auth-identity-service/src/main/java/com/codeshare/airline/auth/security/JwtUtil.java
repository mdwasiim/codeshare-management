...
public class JwtUtil {
    ...
    @Autowired
    public JwtUtil(RoleMapper roleMapper, PermissionMapper permissionMapper, UserMapper userMapper, RolePermissionResolverService rolePermissionResolverService) {
    ...
    // Generate a secure key on startup
    @PostConstruct
    public void init() {
        secret = Keys.secretKeyFor(SignatureAlgorithm.HS512); // 512-bit key
    }
    ...
    public String generateAccessToken(User user) {
        ...
        // Roles
        claims.put("roles", rolePermissionResolverService.resolveRoleNames(user.getId(), user.getTenantId()));

        // Permissions from roles
        claims.put("permissions", rolePermissionResolverService.resolvePermissionsNames(user.getId(), user.getTenantId()));
