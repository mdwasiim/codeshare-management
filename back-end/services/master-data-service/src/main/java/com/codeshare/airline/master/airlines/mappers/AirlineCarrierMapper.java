...
public interface AirlineCarrierMapper extends CSMGenericMapper<AirlineCarrier, AirlineCarrierDTO> {
    ...
    AirlineCarrier toEntity(AirlineCarrierDTO dto);

    @Mapping(target = "country", ignore = true)
    @Mapping(target = "headquartersCity", ignore = true)
    @Mapping(target = "homeAirport", ignore = true)
    @Mapping(target = "alliance", ignore = true)
    void updateEntityFromDto(AirlineCarrierDTO dto, @MappingTarget AirlineCarrier entity);
}
