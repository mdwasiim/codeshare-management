...
@Table(
        ...
        indexes = {
                @Index(
                        name = "IDX_COCKPIT_CREW_EMPLOYER_STATUS",
                        columnList = "STATUS"
                ),
                @Index(
                ...
                @Index(
                        name = "IDX_COCKPIT_CREW_EMPLOYER_ACTIVE",
                        columnList = "ACTIVE"
                )
        }
)
...
@Setter
@NoArgsConstructor
public class CockpitCrewOperator extends CSMDataAbstractEntity {
    ...
    /**
     * Optional when the employer itself is an airline.
     */
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "AIRLINE_ID",
            foreignKey = @ForeignKey(name = "FK_COCKPIT_CREW_EMPLOYER_AIRLINE")
    )
