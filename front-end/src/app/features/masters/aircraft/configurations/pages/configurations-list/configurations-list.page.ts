...
import { AircraftConfigurationFormPage } from '@features/masters/aircraft/configurations/pages/configurations-form/configurations-form.page';

@Component({ selector: 'configurations-list', standalone: true, imports: [CommonModule, TableModule, ButtonModule, TagModule, TooltipModule, ToolbarActionComponent, AppDialogComponent, HasPermissionDirective, AircraftConfigurationFormPage], templateUrl: './configurations-list.page.html' })
export class AircraftConfigurationListPage extends BaseListComponent<AircraftConfiguration> {
    protected override resourceName = 'AIRCRAFTCONFIGURATION';
    ...
    deleteRecord(record: AircraftConfiguration) {
        ...
        this.confirm.delete('Delete ' + this.recordLabel(record) + '?', () => {
            this.service.delete(record.id!).subscribe({ next: () => this.refresh(), error: () => this.toast.error('Failed to delete Configurations') });
        });
    }
    deleteSelectedRecords() {
        ...
        this.confirm.delete('Delete ' + this.selectedRecords.length + ' selected record(s)?', () => {
            ...
            forkJoin(requests).subscribe({ next: () => { this.selectedRecords = []; this.refresh(); }, error: () => this.toast.error('Failed to delete selected Configurations') });
        });
    }
    ...
    recordLabel(record: AircraftConfiguration) { return String((record as any).name || (record as any).displayName || (record as any).countryName || (record as any).legalName || (record as any).code || record.id || 'record'); }
}
