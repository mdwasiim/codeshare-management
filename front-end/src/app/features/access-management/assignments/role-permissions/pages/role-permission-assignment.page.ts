...
import { RolePermissionService } from '@features/access-management/assignments/role-permissions/services/role-permission.service';
...
import { GroupRoleService } from '@features/access-management/assignments/group-roles/services/group-role.service';
...
export class RolePermissionAssignmentPage implements OnInit {
    ...
    // ROLE SELECT
    // =========================
    onRoleSelect(event: TableRowSelectEvent): void {
        ...
        this.service.getPermissionsByRole(role.id).subscribe({
            next: (res) => {
                this.selectedPermissionIds = res.map((x) => x.permissionId);
    ...
    togglePermission(permissionId: string | undefined, checked: boolean): void {
        ...
        this.selectedPermissionIds = this.selectedPermissionIds.filter(Boolean);
    ...
    save(): void {
        ...
        this.service.replaceRolePermissions(this.selectedRole.id, this.selectedPermissionIds).subscribe({
            next: () => {
