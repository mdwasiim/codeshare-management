...
import { Group } from '@features/access-management/models/group.model';
import { AppToastService } from '@services/toast/app-toast.service';
import { Component, inject, OnInit } from '@angular/core';
import { GroupMenuService } from '@features/access-management/assignments/group-menus/services/group-menu.service';
...
export class GroupMenuPage implements OnInit {
    private service = inject(GroupMenuService);
    ...
    loadData(): void {
        ...
        forkJoin({
            ...
            next: (res) => {
                ...
                this.menuTree = this.layoutMenuService.buildTree(this.menus);
    ...
    onGroupSelect(group: Group): void {
        ...
        this.service.getMenusByGroup(group.id).subscribe({
            next: (menus) => {
                const ids = menus.map((menu) => menu.id!).filter(Boolean);

                const tree = this.layoutMenuService.buildTree(this.menus);

                this.menuTree = this.toTreeNodes(tree);

                this.expandAll(this.menuTree);

                this.selectedMenus = this.findSelectedNodes(this.menuTree, ids);
    ...
    toTreeNodes(menus: AppMenuModel[]): TreeNode[] {
        return menus.map((menu) => ({
            ...
            children: menu.items?.length ? this.toTreeNodes(menu.items) : []
        }));
    }
    ...
    save(): void {
        ...
        this.service
            .replaceGroupMenus(
                ...
                this.selectedMenus.map((node) => node.key!)
            )
            .subscribe({
                next: () => {
                    this.toast.success('Group menus updated successfully');
                ...
                error: (err) => {
                    ...
                    this.toast.error('Failed to update menus');
    ...
    findSelectedNodes(nodes: TreeNode[], ids: string[]): TreeNode[] {
